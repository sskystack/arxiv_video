...
def parse_pdf_text_for_oa(major_intro_chunk, top_ss_chunks):
    ...
    try:
        ...
        return intro_parse_resp
    except OutputParserException as e:
        print(f'OA Checker Parser Error: {e}. || {top_ss_chunks} ')
        return None


def github_checker(arxiv_id):
    print(f'checking {arxiv_id} for OA')

    pdf_pth = os.path.join(base_analysis_dir, f'{arxiv_id}.pdf')

    if not os.path.isfile(pdf_pth):
        return False

    major_intro_chunk, top_ss_chunks, pdf_corrupted = chunk_pdf_for_oa(pdf_pth, paper_id)
    if pdf_corrupted:
        return False

    naive_parse_resp, intro_parse_resp, parse_error = parse_pdf_text_for_oa(
        major_intro_chunk,
        top_ss_chunks,
    )
    if parse_error:
    ...
    # === Step 3: Rule-based OA Decision ===
    intersection_result = list(
        set(naive_parse_resp.OA).intersection(intro_parse_resp.OA)) if naive_parse_resp and intro_parse_resp else []
