...
def test_mixed_language_audio():
    ...
    try:
        # 生成音频
        logger.info("� 开始生成混合语言音频")
        audio_path = tts_service.generate_audio(card.info_CN, str(output_dir))
        
        if audio_path and os.path.exists(audio_path):
            logger.info(f"✅ 成功生成混合语言音频: {audio_path}")
            # 显示文件大小
            ...
            for i, audio_file in enumerate(audio_files):
                size = os.path.getsize(audio_file) / 1024  # KB
                logger.info(f"   {i+1}. {audio_file.name} ({size:.1f} KB)")
        else:
            logger.error("❌ 音频生成失败")
